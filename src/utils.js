...
export const filterData = ({ data, errorFilter, filter = {}, search = {} }) => {
  ...
  return !trimmedSearch && !filter.name && !errorFilter
    ? data
    : data.filter((info) => {
        let isRegexMatched = true;
        let isSearchMatched = true;

        if (trimmedSearch.startsWith("/") && trimmedSearch.endsWith("/")) {
          const regexPattern = trimmedSearch.slice(1, -1);
          const regex = new RegExp(regexPattern);
          isRegexMatched = info[search.name] && regex.test(info[search.name]);
          console.log(regex, info[search.name], isRegexMatched);
        } else {
          isSearchMatched = trimmedSearch
            ? info[search.name] && info[search.name].includes(trimmedSearch)
            : true;
        }
        ...
        return isRegexMatched && isSearchMatched && isErrorMatched && isFilterMatched;
      });
};
